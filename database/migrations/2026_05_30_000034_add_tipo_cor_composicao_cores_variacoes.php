...
return new class extends Migration
{
    public function up(): void
    {
        ...
        // FK pcc_composicao_fk usa o índice único composto; criar índice dedicado antes de removê-lo.
        Schema::table('produto_composicao_cores', function (Blueprint $table) {
            $table->index('id_composicao', 'pcc_composicao_idx');
        });

        Schema::table('produto_composicao_cores', function (Blueprint $table) {
            $table->dropUnique('pcc_composicao_item_cor_unico');
        });

        Schema::table('produto_composicao_cores', function (Blueprint $table) {
            $table->string('tipo_cor', 20)->default('PRIMARIA')->after('id_item_projeto');

            $table->unique(
                ['id_composicao', 'id_item_projeto', 'tipo_cor', 'id_cor'],
                'pcc_composicao_item_tipo_cor_unico'
            );
        });
        ...
        Schema::create('produto_variacoes', function (Blueprint $table) {
            ...
            $table->unique(
                ['id_composicao', 'id_item_projeto', 'tipo_cor', 'id_cor'],
                'pv_composicao_item_tipo_cor_unico'
            );
        });
